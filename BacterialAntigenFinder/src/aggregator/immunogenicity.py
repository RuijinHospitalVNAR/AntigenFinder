...
整合Vaxign-ML结果和VaxiJen抗原性计算进行蛋白质级别的免疫原性评估。

更新: 整合完整VaxiJen 2.0算法进行抗原性评估
"""
...
class ImmunogenicityResult:
    """免疫原性评估结果"""
    protein_id: str
    protegenicity_score: float  # Vaxign-ML保护性分数
...
class ImmunogenicityEvaluator:
    ...
    def evaluate(self, 
                 vaxignml_result: PredictionResult,
    ...
    def _rank_and_recommend(self, 
        ...
        for result in results:
            # 综合评分 = 保护性分数 * 位置权重 * 抗原性分数
            location_weight = self._get_location_weight(result.subcellular_location)
            
            # 跨膜蛋白降权
            tm_penalty = 0.8 if result.transmembrane_regions > 2 else 1.0
        ...
        for i, result in enumerate(results):
            ...
            # 确定推荐等级
            if result.is_protective_antigen and \
               result.subcellular_location in self.PREFERRED_LOCATIONS and \
               result.antigenicity_score >= self.antigenicity_threshold:
                result.recommendation = 'HIGH'
            elif result.is_protective_antigen or \
                 result.subcellular_location in self.PREFERRED_LOCATIONS:
