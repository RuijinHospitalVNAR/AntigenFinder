        ...
        try:
            ...
            for idx, (_, row) in enumerate(df.iterrows(), start=1):
                ...
                if residue_info:
                    ...
                    try:
                        # Use explicit residue id if present; otherwise fall back to
                        # 1-based row index so predictions align with other predictors.
                        residue_id = int(parts[-1]) if len(parts) > 1 else idx
                    except ValueError:
                        residue_id = idx
                else:
                    residue_name = 'X'
