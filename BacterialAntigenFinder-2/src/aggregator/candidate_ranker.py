...
class CandidateRanker:
    ...
    def _determine_epitope_recommendation(self,
        ...
        protegenicity 由 VaxiJen 2.0 抗原性分数映射而来 (0-100)，
        antigenicity 为原始 VaxiJen 分数 (0-1)。

        Args:
            composite_score: 综合评分
            avg_consensus_score: 平均共识分数
            protegenicity: 保护性分数（VaxiJen映射，0-100）
            antigenicity: VaxiJen抗原性分数 (0-1)

        Returns:
            推荐等级: HIGH / MEDIUM / LOW
        """
        if composite_score >= 0.6 and avg_consensus_score >= 0.5 and antigenicity >= 0.5:
            return 'HIGH'
        elif composite_score >= 0.4 and (avg_consensus_score >= 0.3 or antigenicity >= 0.4):
            return 'MEDIUM'
        else:
            return 'LOW'
