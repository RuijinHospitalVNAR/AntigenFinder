...
示例用法:
  ...
  python main.py --config my_config.yaml --fasta antigens.fasta
    ...
    model_group.add_argument(
        '--models', '-m',
        type=str,
        default='bepipred,discotope,graphbepi,epigraph',
    ...
    model_group.add_argument(
        '--consensus_method',
        type=str,
        choices=['weighted_avg', 'majority_vote', 'max_score', 'mean'],
        default='weighted_avg',
        help='共识评分方法 (默认: weighted_avg)'
    )
    model_group.add_argument(
        '--score_normalize',
        type=str,
        choices=['none', 'rank'],
        default='rank',
        help='跨模型分数归一化: rank=百分位排名(推荐), none=原始分数 (默认: rank)'
    )
...
class AntigenFinderPipeline:
    ...
    def __init__(self, config: dict):
        ...
        self.consensus_scorer = ConsensusScorer(
            ...
            min_votes=config.get('consensus', {}).get('min_votes', 2),
            method=config.get('consensus', {}).get('method', 'weighted_avg'),
            score_normalize=config.get('consensus', {}).get('score_normalize', 'rank')
        )
    ...
    def run(self, fasta_file: str, pdb_dir: str, organism_type: str, 
        ...
        # Step 4: 免疫原性评估
        self.logger.info("Step 4: 免疫原性评估 (VaxiJen 2.0)...")
        immunogenicity_scores = self.immunogenicity_evaluator.evaluate_from_sequences(sequences)
        self.logger.info(f"  免疫原性评估完成，{len(immunogenicity_scores)} 个蛋白质")
...
def main():
    ...
    config.setdefault('consensus', {})['method'] = args.consensus_method
    config.setdefault('consensus', {})['score_normalize'] = args.score_normalize
